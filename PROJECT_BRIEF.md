> **2026-09-22 â€” partially superseded.** The user had a real, further-along implementation of
...
> of truth for the app itself â€” read those first. The from-scratch MVP task sequence below is
> archived at `docs/tasks/_archived-scratch-plan/` and no longer active. What's still live here:
> the **licensing policy**, the **repo research** (erdb, save editor, etc. â€” cross-check against
...
# ER Master Tool â€” Project Brief
...
each started fresh with no memory of prior runs â€” every task file must be self-contained.
...
calculator, and (differentiator) real save-file import â€” all in the browser, no server, no
account.

## Locked decisions

- **First shippable slice**: Unified workspace MVP â€” Reckoning (the front door: interview +
  ...
  provenance. Everything else designs in around this. See "Reckoning â€” the front door" below.
- **World coverage v1**: Base game + Shadow of the Erdtree (official DLC). "Tarnished Pack" is
  ...
  reference app â€” see below), not a third-party mod.
...
- **Stack**: React + Vite + TypeScript. Tailwind for styling. Plain `@tanstack/react-router`
  (NOT `@tanstack/react-start` â€” no SSR, no server functions, this is a pure client SPA).
...
- **Mobile path**: Ship the PWA first; wrap with **Capacitor** later for iOS/Android app-store
  builds from the same codebase. Do not adopt React Native/Expo â€” that would fork the codebase.
...
## Legacy reference (do not build on top of this â€” mine it, don't extend it)
...
It is **not the foundation** for the new build â€” it's a mining source:

- `src/data/catalog/*.json` â€” weapon/armor/talisman/spell/spirit/ashes data, already compiled from
...
- `src/routes/*.tsx`, `src/components/shell.tsx`, `src/store/ledger.ts` â€” working UX patterns
...
- `public/atlas/*.jpg` â€” AI-generated region illustration images already in the top-level
...
| Build/AR calculator reference | [ThomasJClark/elden-ring-weapon-calculator](https://github.com/ThomasJClark/elden-ring-weapon-calculator) | MIT | React+TS+Vite (same stack). No packaged library â€” port the AR/scaling formulas, credit in NOTICES. |
| Save file import (differentiator feature) | [yo-repo87/ER-Save-Editor](https://github.com/yo-repo87/ER-Save-Editor) | MIT/Apache-2.0 dual | Rustâ†’WASM, runs fully client-side, **nothing uploaded**. Lets a user import their real save to auto-populate the Ledger instead of manual checkbox tracking. |
| Quest lockout / fork tracking | [yosoyelfede/elden-ring-questline-map](https://github.com/yosoyelfede/elden-ring-questline-map) | MIT | Interactive questline dependency map â€” matches the "Weave" feature concept. |

Found but **not yet license/activity-verified** â€” DeepSeek should re-check before integrating:

- `googleben/ERMapViewer` â€” 3D in-game map viewer (Phase 2 candidate, heavier lift)
- `F3T1W/EldenTracker`, `CreateDD` interactive map (itch.io) â€” alternate self-hosted map candidates
- `shishir0x/EldenRing_Map` â€” turned up in search as a self-hosted MapGenie alternative but **404'd on fetch** (may be renamed/deleted/private) â€” do not rely on it without re-confirming it exists
- `CyberGiant7/Elden-Ring-Automatic-Checklist` â€” checklist auto-driven by save data, relevant precedent for the save-import + checklist integration
- `jtross6/EldenRingBuildRandomizer` â€” build randomizer, Phase 2 feature candidate
- `EldenForge/EldenForge_API` â€” broad REST API covering nearly every data category, redundant with erdb but worth a coverage diff if erdb is missing something

## Licensing policy (relaxed 2026-09-22 â€” personal, educational project)

The user has confirmed this is a **personal, educational, non-commercial, non-distributed**
project and is fine with using data available on the internet generally â€” don't gate ingestion on
...
- Don't reproduce another author's specific creative prose (wiki descriptions, a fan walkthrough's
  narrative text) verbatim as if original â€” paraphrase or omit it. This is about not passing off
...
- Respect this project's own stated non-goals regardless of source licensing: don't bulk-ship
  FromSoftware's actual map/tile archives, don't invent attack-rating numbers, etc. â€” see
...
- Still worth a brief note in `THIRD_PARTY_NOTICES.md` for anything substantial that was ported
  or vendored, as a simple provenance record â€” not a compliance gate, just good practice.
- If this project's scope ever changes toward public release or commercial use, revisit this.

## MVP feature scope (Tasks 01â€“07)

1. **Scaffold** (Task 01) â€” clean Vite/React/TS PWA, no backend.
2. **Data layer** (Task 02) â€” broad item/boss/location catalog from erdb.
3. **Knowledge fact graph** (Task 03) â€” narrow hand-seeded fact graph + `Character` state with
   per-fact provenance (`"answer" | "screenshot" | "inference" | "save"`).
4. **Reckoning** (Task 04) â€” the front door: four-question interview + screenshot triage +
   name-matching + inference, with a receipts view. Default pane on app open. Rail entry point
   (stub) for PC save-drop.
5. **Core workspace** (Task 05) â€” Ledger view over the `Character` state, Atlas (region POIs),
   Codex (searchable item database).
6. **Forge** (Task 06, not yet task-briefed) â€” build calculator: attack rating by stats/upgrade
   level, weapon comparison, ported from ThomasJClark's formulas.
7. **PWA polish** (Task 07, not yet task-briefed) â€” manifest/icons, offline caching strategy,
...
not a separate ad hoc checklist bolted on later â€” this was folded in once Reckoning's provenance
...
## Reckoning â€” the front door (added 2026-09-21)

PS5 cannot hand this app a save file (`ER0000.sl2` is PC-only). Reckoning is how a console
player still gets current â€” by interview, screenshot triage, and inference â€” while PC players
...
(including Heavy Knight / Idus Knight); how far the map has opened (Limgrave â†’ Shadow); which
shardbearers are dead.

**Screenshot triage** â€” the specific screens worth capturing, in priority order: Warp list
(best single capture â€” every named grace the player can sit at), Map (gold pins vs. fog),
...
**Inference examples** (facts imply other facts â€” this is the core mechanic, not a nice-to-have):
- Godrick's Great Rune â†’ Godrick is dead â†’ Margit is dead â†’ Stormveil gate graces exist.
- Fingerslayer Blade â†’ Radahn festival happened â†’ Ranni's Nokron step is done.
- Scadutree Fragment (presence of) â†’ player is in the Realm of Shadow.
...
needles, etc.). Each fact entry lists what else must already be true for it to hold â€” that's the
implication graph. Growing this to full coverage (every grace, every item) is mechanical once a
PC game-file extract is running on any one machine â€” the seed catalog is enough to start.

**Provenance ("receipts") is mandatory**: every tick/fact in the Character state must record its
source â€” `answer` (interview), `screenshot` (user-confirmed screen type + typed names),
`inference` (derived from another fact), or `save` (parsed from a PC `.sl2` file). Never invent
a fact with no source.

**What's NOT built yet â€” don't fake it**: on-device OCR of a raw screenshot is a later wiring
pass (Tesseract, cropped to the pickup-banner region and the warp-list column). The MVP loop is
manual and still real: screenshot stored in-tab â†’ player confirms which screen type it is â†’
player types the names they can read â†’ matcher â†’ inference receipts shown. Do not stub in fake
OCR results or silently skip the confirm/type step.

## Phase 2/3 â€” AI voice/chat companion (added 2026-09-21, NOT part of the MVP task sequence yet)
...
full so the design survives until it's actually task-briefed â€” do not start building this before
Tasks 01â€“0N (MVP + Reckoning) are done and reviewed.

**Three separate systems, not "an AI"**: Knowledge (items/bosses/quests/locations/spoiler
rules â€” this is `src/data/catalog/` + `src/knowledge/catalog.ts`, already covered), Map UI (pan/
zoom/highlight/layers â€” the Atlas, already covered), Voice loop (listen â†’ understand â†’ act on
map â†’ speak back â€” new).

**Architecture**: player voice/text â†’ speech-to-text â†’ backend (retrieves facts via structured
lookup + RAG over chunked wiki text, sends to an LLM with chat history) â†’ LLM returns a spoken
answer plus tool calls â†’ app applies the tool calls to the map â†’ text-to-speech reads the
answer. The model never invents coordinates in prose â€” it calls functions, and the app owns map
state:
...
privacy, or very high volume â€” none of which apply at MVP. Swapping providers later shouldn't
require an app rewrite if the tool-call contract stays stable.

**Grounding (mandatory â€” a generic chatbot will hallucinate item locations)**:
- Structured facts (locations, coords, item/boss IDs, region names) come from
  `src/data/catalog/` (erdb) â€” the model looks these up, never guesses them.
...
- Spoiler control is mandatory: store player progress (region unlocked, bosses dead â€” this is
...
backend" â€” flag to the user before starting this phase, don't silently introduce it): auth/save
...
a small VPS) â€” the heavy model stays at the API provider, no GPU hosting needed at this stage.

**Build order** (the user's own sequencing â€” follow it, don't reorder): (1) web map + search box
+ marker highlight â€” this is the MVP Atlas/Codex; (2) text chat that can call `focus_location` /
...
## Resolved â€” PC extract engine identity (2026-09-22)
...
local game install**, never redistributed â€” this is already how `HANDOFF-CLAUDE.md` describes
it ("tiles/markers from a local game install, not shipped"), so no policy change needed.

**Resolved 2026-09-22**: a local Elden Ring install now exists on this machine â€”
...
`SOURCE-PACK.md` previously said was completely unavailable â€” `vendor/elden-ring-map`'s
`npm run map:setup`, erdb's own extraction, and potentially the 9974 mod's `regulation.bin`
(moot now â€” this is a clean unmodified one). `artifacts/all-knowing/vendor/elden-ring-map/` still
needs to be cloned in (excluded from the original handoff zip), but `npm run map:setup`
...
completion mod, Medusa walkthrough, EldenRingMapV1.2) are under background analysis â€” in
...
Node+Python source tree) suggest it's a **different** tool â€” likely still valuable as an
...
criteria before the next task starts â€” do not chain tasks blind.
