...
```
Desktop  280px Now (Gideon) â”‚ Stage 1fr
...
- Three states via `factState()` â€” the four lists + `deniedFacts`. Evidence alone is not â€œtrueâ€.
- Vault: `localStorage` key `all-knowing.vault.v1`. Packet has no screenshots.
- Merge never replace: `src/lib/merge.ts`.
- Gideon: `askGideon` â†’ `GideonAct`. The shell always switches `act.module`.
...
1. er-guide lat/lng â†’ `public/sourced/open/coords.json`
2. Goblins XYZ + `m60`/`m61` tiles Ã— 256 â†’ `boss-pins.json`
...
once, and the detail panel resolves its target in the active view only (`src/lib/atlasSelection.ts`) â€”
...
`TalkID` -> `NPCParamID` -> name, naming 95 speakers / 1,952 lines. Ceiling is inherent â€” ESDs reference
...
hunts, stuck, 100% spine, boss pins, and (Tasks 74â€“86) region "what did I miss here", wear a named
...
- PC: `npm run map` SSE, or drop `ER0000.sl2` â€” `src/lib/save.ts` is a real read-only in-browser parser.
...
Live list is `HANDOFF-CLAUDE.md` Â§6â€“Â§8. As of Task 86 the open threads are:
