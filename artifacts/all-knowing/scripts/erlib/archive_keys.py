"""Elden Ring dvdbnd (.bhd) archive RSA public keys.

The .bhd headers are encrypted with textbook RSA using a PUBLIC key with a
large exponent, so decryption needs nothing secret. These are the publicly
documented keys shipped by UXM / SoulsFormats.
"""

ARCHIVE_KEYS = {
    "Data0": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----""",
    "Data1": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----""",
    "Data2": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----""",
    "Data3": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----""",
    "DLC": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----""",
}
