...
| `open/dialogue-owners.json` | Speaker attribution for the lines the game's ESD talk scripts reference. `scripts/extract-dialogue-owners.py` probes every `/script/talk/<map>.talkesdbnd.dcx`, scans each named ESD (`t<4-digit NpcParam family>…`) for real `TalkParam` ids, and joins through `TalkParam.msgId` to the `TalkMsg` line. **Partial by design:** 2,129 of 9,818 lines across 157 families, 56 of them named (name = the family's lowest NpcParam row, from `NpcParam.txt` + `npc-combat.json` + `enemy-combat.json`). An unnamed family is shown as `npc <prefix>`; cutscene/menu lines are simply not attributed — no speaker is invented. `src/lib/dialogueOwners.ts` (`speakerLabel`). |
