...
| `open/dialogue-owners.json` | Speaker attribution for the lines the game's ESD talk scripts reference. `scripts/extract-dialogue-owners.py` matches each ESD's `t<talkId>.esd` to the MSB PARTS entry carrying that **TalkID** (at `+0x2b0`), reads its **NPCParamID** (`+0x2a8`), and names it from `NpcParam.txt` + the combat JSONs — the way the game itself resolves a talker. **Partial by design:** 1,950 of 9,818 lines across 95 named speakers. Cutscene/menu lines and NPCs with no placed MSB part are not attributed; nothing is guessed. `src/lib/dialogueOwners.ts` (`speakerLabel`). |
