...
- `maps/m0-overworld.jpg` â€” Pack 960 full official mosaic, 4096x3880 (~4.4 MB)
- `maps/m1-underground.jpg` â€” Pack 960 underground, 4096x3880 (~0.7 MB)
- `maps/m-ashen.jpg`, `maps/m-shadow.jpg` â€” AI-generated stand-in art (no real
...
- Static-plate pins (`open/coords.json`, `open/boss-pins.json`) use the engine
  ...
  plotted on the static plate â€” XYZ only, 10k rows, would clutter and add a
  third projection. See `src/lib/coords.ts`.

## Attack rating (Build lab)

- `regulation-vanilla-v1.17.json` â€” ThomasJClark/elden-ring-weapon-calculator's vanilla 1.17 /
...
- **Regenerate the FMG name dump** from a local install:
  `python scripts/extract-fmg-names.py` â€” reads the install's `item` + `item_dlc02` FMG and
...
- **Top up the Paramdex equipment names** from a local install:
  `python scripts/extract-paramdex-names.py` â€” appends only the ids the upstream dump is missing
...
- **Regenerate the atlas markers** from a local install:
  `python vendor/elden-ring-map/tools/build_markers.py "<game dir>"` â€” writes the
  gitignored `vendor/elden-ring-map/data/markers.json` (1,106 markers incl. Shadow of the Erdtree).

## Boss combat (Build lab) â€” real NpcParam

- `npc-combat.json` â€” 83 named bosses keyed to catalog fact ids: per-damage-type damage negation,
...
- `baseHp` and `resist` are raw param values before the game's area/NG scaling â€” `baseHp` is not
...
## Regular enemy combat (Build lab) â€” real NpcParam + MSB

- `enemy-combat.json` â€” 2271 regular (non-boss) enemies keyed `enemy:<npcRow>`, with the same
  ...
  2. MSB `PARTS_PARAM_ST` enemy parts give the authoritative `NPCParamID` per placement (located
     by matching ints against the real NpcParam id set â€” the field sits at entry +0x2ac, falling
...
blessings â€” category `scadutree-fragment` (**50** rows) and `revered-spirit-ash` (**25** rows).
...
and print `Lv â€”` rather than an invented level.

ER Checklist (Nexus 9953, `sote-1.16`) â€” the pack's `mods/ercl/items.json` is a plain
...
| `open/ercl-items.json` | ER Checklist (Nexus 9953) item ids/names, 1.16 base + SotE (`mods/ercl/items.json`): 154 talismans, 129 incantations, 116 ashes of war, 104 cookbooks, 84 sorceries / spirit ashes, 50 gestures, 40 crystal tears, 6 whetblades â€” broader than the base-game FanAPI lists. `scripts/ingest-packs.py`; `src/lib/packs.ts`. |
| `sourced/npc-placements.json` | Every placed NPC/enemy and its world position, from the map MSBs (`scripts/extract-npc-placements.py`): 26,997 placements, 912 distinct NPCs, 95 flagged `dialogue`. Position is the part's raw local position; `src/lib/npcPlacements.ts` matcher. Gideon "where is X" falls back to it (maps) when the authored stage table has no entry. |
...
| `open/msb-enemies.json` | 8.8k placed enemies â€” joined to `enemy-combat.json` for placement counts/maps |
| `open/gathering-nodes.json` | 21.8k AEG gathering-node placements (Goblins dump); model code only, no item/material field â€” `src/lib/gatheringNodes.ts` labels them honestly as generic placements, world resolved only for the two confirmed grids (area 60 â†’ overworld, 61 â†’ shadow), everything else buckets as `underground` unverified. **Task 62:** never drawn as Atlas pins and never answered by Gideon; Codex-only, labelled "unverified placement, model code only", guarded by `gatheringNodes.guard.test.ts` |
...
| `open/fanapi/*.json` | **Task 67/68** structured FanAPI reference (all 14 categories): `armors.json` (568 â€” poise, negation, resistance, weight), `talismans.json` (87 â€” effect), `spells.json` (169 â€” sorcery+incantation cost/slots/requires/effect), `ashes.json` (90 â€” skill/affinity), `spirits.json` (64 â€” FP/HP/effect), `items.json` (462 â€” type/effect), `locations.json` (177 â€” region), `creatures.json` (115 â€” location/drops), `bosses.json` (106 â€” region/location/HP/drops), `npcs.json` (55 â€” location/role), `ammos.json` (53 â€” type/passive), `classes.json` (14 â€” level/stats), `weapons.json` (307) + `shields.json` (69 â€” category/weight only). Produced by `scripts/ingest-fanapi.mjs` (deterministic, name-sorted); loaded by `src/lib/fanapiData.ts`, surfaced in the Codex. Structured fields only â€” no article bodies/images, and **no attack/defence numbers** (AR stays on the in-repo regulation data). Base-game only; FanAPI predates SotE. |

## Alias plane (generated)

- `public/sourced/aliases.json` + `src/data/aliases.json` â€” one generated table mapping engine
  row id (`grace:100000`, `bossflag:510010`, `npc:21300014`, `goods:8175`, â€¦) â†’ catalog slug â†’
  FMG name â†’ aliases, across grace/boss/invader/hunt/item/quest/region. The two files are
...
- **Task 55 completeness pass:** the generator also reads `checklists/hunts.json` (kind `hunt`) and
  ...
  (`grace:120208` â†’ `grace:night-sacred-ground`). Strict parenthetical-preserving matching makes
...
- **Task 73 warp stubs:** every remaining warp gets a name-derived `grace:{slug}` alias row
  (`source: 'grace-stub'`) â€” no catalog fact, no pin, no implications â€” and an authored id still
  wins on a collision. Output is now **1273 rows / ~274 KB**; **unmatched warps 360 â†’ 0**, bosses
...
FanAPI JSON (weapons, armors, spells, â€¦) + `hunts.json` (207 flags) + `graces.json`.
...
(deleted in Task 37 â€” it could drift). See `docs/research/hunt-data-cleanup.md`.

## Item / boss images (generated)

- `public/sourced/images/<category>/<id>.webp` â€” 2,244 FanAPI thumbnails (160 px WebP,
...
- `src/data/image-index.json` â€” normalised name â†’ local thumbnail path (~2.1k names). Imported
...
- `dungeons.ts` â€” the Stormveil checklist (Task 80): 8 beats on the three Stormveil-region graces,
  rendered by `src/Dungeon.tsx`; ticking goes through `applyFacts` / `clearFact`.
- `npcLocations.ts` â€” 8 companions Ã— staged `{ graceId, whenFacts }` rows (Task 79); every grace id
  is validated against warps âˆª catalog âˆª aliases, invalid rows are skipped and reported.
- Lib helpers alongside: `src/lib/{beatPins,regionLeftovers,gideonHeader,coop}.ts` (Tasks
  75/72/78/81).

## Self-hosted fonts (Task 58)

`public/fonts/*.woff2` â€” 6 files (~173 KB): Cinzel and Source Sans 3, both SIL OFL 1.1, latin +
latin-ext subsets. Both families ship as variable fonts, so one file per family/style/subset
(`cinzel-normal-latin`, `source-sans-3-normal-latin`, `source-sans-3-italic-latin-ext`, â€¦) serves
...
- `src/knowledge/npc-display.ts` â€” player-model NPC level/stat allocation from the EanNewton
...
## Gates + wiki-grade lines (Tasks 52â€“53)

- `src/knowledge/gates.ts` â€” ten authored world-state gates (Forge, Maliketh, Sealing Tree,
  Ranni / frenzy / dung-eater ending commits, Seluvis potion, Volcano Manor, Millicent's Elphael
  fork, VarrÃ©). Not extracted data: each gate is authored from `missables.ts`,
...
- `src/lib/gatePins.ts` â€” binds approaching gates' locks onto the Atlas plate using only the two
  existing frames (lootâ†’grace, `coords.json`). Unplaced locks are listed in the side panel.
- Task 53 added 54 authored catalog facts (49 `quest:` state ids across the eight lines, plus
  ...
  count 226 â†’ 286. These are quest state, not extracted `regulation.bin` flags â€” no event flag id
  is invented. `src/knowledge/storylines.test.ts` carries the Task 53 golden fixture.

## Inference chains (Task 54)

- `src/knowledge/inferChains.ts` â€” an authored `{ whenFact, implies, allOf?, unless?, confidence,
...
- Seeds: Fingerslayer â†’ Ranni Nokron beat; each Great Rune â†’ its own boss only; Black Knifeprint â†’
  Rogier's beat; Pureblood Medal â†’ VarrÃ© cloth; Mimic Tear Ashes â†’ Mimic Tear; Black Whetblade â†’
  Night's Sacred Ground; Twinned set â†’ `quest:fia:dagger`; the two Haligtree medallion halves â†’
