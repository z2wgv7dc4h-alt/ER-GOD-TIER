...
| `open/bosses-fextralife.json` | Boss drops + per-location runes, scraped from the Fextralife wiki (`scripts/scrape-fextralife-bosses.mjs`): **163 bosses, 161 with drops**, base + Shadow of the Erdtree — closes the DLC gap the FanAPI/checklist boss files had. Each row keeps its page URL; loaded by `src/lib/bosses.ts`, surfaced in the Codex. |
