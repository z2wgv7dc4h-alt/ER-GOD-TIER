...
| `sourced/npc-placements.json` | Where each **talking** NPC stands, from the map MSBs (`scripts/extract-npc-placements.py`): 1,370 placements across the 95 dialogue NPCs. Enemy spawns are deliberately excluded (tens of thousands of respawning mobs are noise; that set is `open/msb-enemies.json`). Position is the part's raw local position; `src/lib/npcPlacements.ts`; Gideon "where is X" falls back to it. |
