...
| `open/engine-markers.json` | The map engine's own named data (`scripts/export-engine-markers.mjs`): 1,106 world markers (413 graces + bosses/landmarks/poi/fragments) and **3,347 named item pickups** with nearest Site of Grace, in the engine frame (`px/10496*100`). Canonical source for pins + "where to find" (`src/lib/engineMarkers.ts`); the Nexus "EldenRingMap" pack is the same project and was removed as a duplicate. |
| `open/save-ids.json` | Elden Ring Save Editor (alfizari) name↔hex-ID tables (`scripts/scrape-save-editor.mjs`): 959 weapons, 729 armor, 154 talismans, 116 Ashes of War, 1,850 goods, plus graces/bosses/maps. `src/lib/saveIds.ts`. |
| `open/acquisition.json` | Item acquisition ripped from the elden-ring-mcp wiki-snapshot DB (`scripts/export-mcp-db.py`): **2,609 rows** — method (drop/chest/merchant/ground/quest), location, nearest grace, prerequisites, missable. `src/lib/acquisition.ts`; used by Gideon `find_item`. |
| `open/npc-quests.json` | NPC quest walkthroughs from the same DB: **68 NPCs / 341 ordered steps** with location, action and `breaks` flag. `src/lib/npcQuests.ts`; Gideon `quest_steps`. |
| `open/recipes.json` | Crafting recipes from the same DB (`scripts/export-mcp-db.py`): **124 craftable items** with material quantities (from each item page's "Required items" section). `src/lib/recipes.ts`; Gideon `recipe`. |
| `open/wiki-sections.json` | Full wiki prose from the same DB: **19,174 sections** (~5.9 MB) across the wiki. The "answer anything" text layer. `src/lib/wikiText.ts`; Gideon `wiki` tool. |
| `open/secrets.json` | Illusory / hidden walls and what is behind them, from the same DB: **46 entries** with area + text. `src/lib/secrets.ts`; Gideon `secrets` tool. |
...
| `open/bosses-fextralife.json` | Boss pages scraped from Fextralife (`scripts/scrape-fextralife-bosses.mjs`): **163 bosses, 161 with drops**, base + SotE — drops, per-location runes, HP, and the page body (**strategy / combat / lore**, cutscene dialogue skipped). `src/lib/bosses.ts`; Codex. |
